class CharTreeGraph {
    class CharTreeNode {
        ...
        boolean addStringToGraph(String str, Token key, boolean ignoreValidity) {
            ...
            for (int i = 0; i < currentChild; i++) {
                if (children[i].c == str.charAt(0)) {
                    ...
                    if (str.length() > 1) {
                        str = str.substring(1);
                        return children[i].addStringToGraph(str, key, ignoreValidity);
                    } else {
                        if (children[i].key != null) {
                            if (children[i].key.isValid && children[i].isEnd) {
                                if (ignoreValidity) {
            ...
            if (!hasFound) {
                ...
                if (str.length() != 1) {
                    str = str.substring(1);
                    children[currentChild].addStringToGraph(str, key, ignoreValidity);
                }
                currentChild++;
    ...
    boolean addTokenToGraph(Token str, boolean applyValid, boolean ignoreValidity) {
        if (str != null) {
            str.isValid = applyValid;
            return node.addStringToGraph(str.tokenName, str, ignoreValidity);
        }
        return false;
    }
